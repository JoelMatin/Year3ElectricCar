...
//calcule le champs reu en chemin direct sans obstacle
std::complex<float> champIncident(emetteur Source, point Recepteur, float d) {
    ...
    std::complex<float> expo= std::exp(- beta * d * std::complex<float> (0,1));
...
//calcule le champs de la rflexion sans obstacle
...
std::complex<float> champDoubleReflex(const emetteur& Source, const point& Recepteur, const mur& MurReflex1,
   ...
   // mur Mur, point Recepteur, emetteur Source, point pSource, std::complex<float> Zm, std::complex<float> gm, point sym
