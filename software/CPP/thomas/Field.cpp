...
std::complex<double> Field::E_transmission() {
    ...
    std::complex<double> field = Tm * sqrt(60.F * TX.find_GTX3(direction.p2) * TX.PTX) * exp(-1.0 * i() * TX.beta * d) / d;
    return field; 
}
...
std::complex<double> Field::E_reflection() {
    ...
    get_reflection_walls(reflection_points, sym_points);      //fonction pour trouver mûr
    ...
    for (int k = 0; k < reflection_walls.size(); k ++) {
        ...
        field += Tm_1 * Tm_2 * gamma_m * sqrt(60.F * TX.find_GTX3(direction.p2) * TX.PTX) * exp(-1.0 * i() * TX.beta * d) / d;
    }
    return field; 
}
...
std::complex<double> Field::E_double_reflection() {
    ...
    for (int k = 0; k < double_reflection_walls.size(); k +=2) {
       ...
       /// std::cout << transmission_walls_3.size() << std::endl;
        ...
        field += Tm_1 * Tm_2 * Tm_3* gamma_m1 * gamma_m2 * sqrt(60.F * TX.find_GTX3(direction.p2) * TX.PTX) * exp(-1.0 * i() * TX.beta * d) / d;
    }
    return field;
}
