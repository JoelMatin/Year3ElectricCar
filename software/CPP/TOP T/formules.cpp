...
float calculerPhi(point pSource, point Recepteur) {
    std::vector<float> v{ Recepteur.x - pSource.x, Recepteur.y - pSource.y };  //vecteur entre TX et RX
    vecteurNorme(v); 
    float phi = acos(v[0]); 
    return phi; 
}
...
float calculGTX(emetteur Source, point Recepteur, int number) {
    if (number==2) {
        return 1.7;         //dans le cas du TX2
    }
    if (number==3) {        //dans le cas du TX3
        float phi3dB = M_PI/6; 
        float phi = calculerPhi(Source.coordonnees, Recepteur); 
        float GTXdB = 21.5836 - 12 * pow((phi - Source.delta) / phi3dB, 2); 
        float GTX = (float)pow(10, GTXdB / 10); 
        return GTX; 
