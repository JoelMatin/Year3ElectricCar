...
/*
std::complex<float> champDoubleReflexion(emetteur Source, point Recepteur, mur MurReflex1, mur MurReflex2,
    std::vector<mur> MursTrans1, std::vector<mur> MursTrans2) {
    ...
    point I1 = trouverSym(pSource, MurReflex1); 
    point I2 = trouverSym(I1, MurReflex2); 
    drr = distance(I2, Recepteur); 
    point PtReflexion2 = calculPtReflexion(I1, Recepteur, MurReflex2);     //pas besoin de calculer ptReflexion1
...
float puissanceDoubleReflexion(emetteur Source, point Recepteur, std::vector<std::vector<mur>>& MursReflex) {
    float omega = 2 * M_PI * Source.freq;
    float resistanceSource = Source.resistance;
    float he = Source.he;
    point pSource = Source.coordonnees;
    ...
    for (const std::vector<mur>& paire : MursReflex) {
        mur Mur1 = paire[0]; 
        mur Mur2 = paire[1]; 
